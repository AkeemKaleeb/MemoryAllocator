...
void* mymalloc(size_t size, char *file, int line) {             // function to allocate memory in 8 byte chunks to the heap
    ...
    if(firstChunk->chunkDataSize == 0) {                        // initialize the memory heap if not already                    
        firstChunk->chunkDataSize = MEMLENGTH - (headerSize);  // assign data of chunk to the entire, empty heap
    ...
    while(currentChunk != NULL) {                                                           // loop through all of the chunks
        if(!currentChunk->isAllocated && currentChunk->chunkDataSize >= alignedSize) {      // check if the current chunk is free and large enough
            ...
            if(currentChunk->chunkDataSize >= alignedSize + headerSize) {                   // check if the current chunk can be split into two chunks, makes sure this has enough space for a second header plus the current header and data
                // split the chunk into allocated and free parts
                printf("Splitting chunk\n");
                Chunk *newChunk = (Chunk*)(currentChunk + alignedSize*8);              // point to a new ending chunk after the position of the data length
                ...
                currentChunk->chunkDataSize = alignedSize - headerSize;
